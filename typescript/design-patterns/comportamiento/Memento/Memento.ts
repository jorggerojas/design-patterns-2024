...
class Originator {
  ...
  doSomething(): void {
    console.log("Originator: Está haciendo algo importante");
    this.state = this.generateRandomString(50);
    console.log("Originator: el state cambió", this.state);
  }
...
class ConcreteMemento implements Memento {
  ...
  getName(): string {
    return `${this.state} / (${this.state.slice(0, 5)}...)`;
  }
  getDate(): string {
    return this.date;
  }
...
class Caretaker {
  ...
  backup(): void {
    console.log("Caretake: está guardando el estado de Originator");
